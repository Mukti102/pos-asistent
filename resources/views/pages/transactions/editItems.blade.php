...
@section('content')
<div x-data="transactionHandler({{ $products->toJson() }}, {{ $transaction->items->toJson() }}, {{ $transaction->tax }}, {{ $transaction->discount }}, '{{ $transaction->payment_status }}')" class="w-full">
    
    <form action="{{ route('transactions.update', $transaction->id) }}" method="POST">
        @csrf
        @method('PUT')

        {{-- Gunakan template tabel yang sama dengan create.blade.php --}}
        ...
        {{-- Di bagian Alpine JS Script, ubah function inisialisasinya: --}}
    </form>
</div>

<script>
function transactionHandler(productsData, existingItems, existingTax, existingDiscount, existingStatus) {
    return {
        products_db: productsData,
        // Isi items dengan data dari database jika sedang edit
        ...
        addItem() {
            this.items.push({ product_id: '', price: 0, quantity: 1, total: 0 });
        },
        removeItem(index) {
            if (this.items.length > 1) this.items.splice(index, 1);
        },
        updatePrice(index) {
            const product = this.products_db.find(p => p.id == this.items[index].product_id);
            this.items[index].price = product ? product.selling_price : 0;
        },
        subTotal() {
